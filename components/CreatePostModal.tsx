  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white border border-slate-200 rounded-3xl max-w-lg w-full p-4 sm:p-6 shadow-2xl relative overflow-hidden max-h-[90vh] flex flex-col text-slate-900"
      >
        {/* Modal Header */}
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-900">Create Post</h3>
              <p className="text-[11px] text-slate-500 font-medium">Publish to Social Feed with AI Magic Assistant</p>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-full bg-slate-100 text-slate-500 hover:text-slate-900 hover:bg-slate-200 transition"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none focus:border-purple-500 font-bold truncate flex-1"
              >
          ...
          {postMode === 'text' ? (
            <div className="space-y-3">
              ...
              >
                <textarea
                  ...
                  rows={4}
                  className={`w-full bg-transparent placeholder-slate-400 focus:outline-none resize-none font-medium text-sm leading-relaxed ${
                    selectedGradient !== 'none' ? 'text-white text-base font-bold text-center my-auto placeholder-white/70' : 'text-slate-900'
                  }`}
                />
              </div>
            ...
            <div className="space-y-3">
              <textarea
                ...
                rows={2}
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-purple-500 focus:bg-white resize-none font-medium"
              />
              ...
              {mediaGallery.length > 0 || videoSrc ? (
                <div className="space-y-2">
                  <div className="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                    ...
                    {videoSrc && (
                      <div className="relative rounded-xl overflow-hidden aspect-square bg-slate-900 border border-slate-200 col-span-3">
                ...
                <label className="border-2 border-dashed border-slate-200 hover:border-purple-500 rounded-2xl p-5 flex flex-col items-center justify-center cursor-pointer bg-slate-50 hover:bg-purple-50/20 transition group">
                  <input type="file" accept="image/*,video/*" multiple onChange={handleMediaUpload} className="hidden" />
                  <UploadCloud className="w-8 h-8 text-slate-400 group-hover:text-purple-600 mb-2 transition" />
                  <span className="text-xs font-bold text-slate-700 group-hover:text-purple-900">
                    Upload Photos or Video
                  </span>
                  <span className="text-[10px] text-slate-400 mt-0.5">Supports multi-photo gallery & MP4</span>
                </label>
              )}
            </div>
          )}
        ...
        <div className="pt-3 border-t border-slate-100 shrink-0 flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 text-xs font-bold rounded-xl transition"
          >
            Cancel
          </button>
